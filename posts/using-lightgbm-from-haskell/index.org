...
this interface to the library relatively safe.  In partcular:
  - Many LightGBM parameters are supposed to be limited to defined
    ...
    Haskell interface I use refinement types to try to ensure that
    these limitations are honored - at compile time when possible, at
    run time when necessary.
...
There are several things to note here:  
  - I use the [[https://hackage.haskell.org/package/refined][refined]] library to limit some of the parameters to their
    ...
    I can ensure that it's so at compile time (at the expense of
    enabling the =TemplateHaskell= language extension).
